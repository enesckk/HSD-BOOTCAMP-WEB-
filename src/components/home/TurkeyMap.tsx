...
import { ACTIVE_CITY_POINTS, CityPoint } from '@/utils/constants';
    ...
    <div className={"flex flex-col items-center gap-4 " + (className ?? '')}>
      <div ref={containerRef} className="relative w-[600px] h-[600px] select-none">
        ...
        {cities.map((c, idx) => {
          ...
          return (
            <div key={c.key} className="absolute z-10" style={{ left: `${leftPx}px`, top: `${topPx}px`, transform: 'translate(-50%, -100%)' }}>
              <button onClick={() => setSelected(c)} className="relative pointer-events-auto" aria-label={`${c.name} - ${c.bootcamps} bootcamp, ${c.universities} üniversite`}>
                <motion.div initial={{ opacity: 0, y: -6, scale: 0.8 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.4, delay: idx * 0.02 }} whileTap={{ scale: 0.9 }} className="relative">
                  <motion.span className="absolute -inset-2 rounded-full border-2 border-red-500/40" style={{ zIndex: -1 }} animate={{ scale: [1, 1.6], opacity: [0.5, 0] }} transition={{ duration: 1.6, repeat: Infinity, ease: 'easeOut', delay: idx * 0.02 }} />
        ...
        <AnimatePresence>
          {selected && (() => {
            ...
            return (
              <motion.div initial={{ opacity: 0, x: 6, y: -4 }} animate={{ opacity: 1, x: 0, y: 0 }} exit={{ opacity: 0, x: 6, y: -4 }} transition={{ duration: 0.15 }} className="absolute z-20 bg-white text-gray-900 rounded-xl shadow-2xl overflow-hidden w-64" role="dialog" aria-modal="true" style={styleObj}>
                <div className="px-4 py-3 bg-red-600 text-white flex items-center justify-between">
                  <div className="font-bold">{selected.name}</div>
                  <button onClick={() => setSelected(null)} className="outline-none focus:outline-none hover:opacity-90" aria-label="Kapat">✕</button>
                </div>
                <div className="p-4 text-sm">
                  <div className="flex items-center justify-between mb-2"><span className="text-gray-600">Bootcamp sayısı</span><span className="font-semibold">{selected.bootcamps}</span></div>
                  <div className="flex items-center justify-between"><span className="text-gray-600">Üniversite sayısı</span><span className="font-semibold">{selected.universities}</span></div>
                </div>
              </motion.div>
            );
          })()}
